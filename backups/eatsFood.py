...
class fishTank(ShowBase):
    ...
    def placePiece(self):
        ...
        for fish in self.fishList:
            fish.feed(self.flake, self.flakeX, self.flakeY, self.flakeZ)
        self.mousing = False
    ...
    def feeding(self, task):
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            self.flakeX=base.mouseWatcherNode.getMouseX() * 35
            self.flakeY=base.mouseWatcherNode.getMouseY() * 25
            self.flake.setPos(self.flakeX, self.flakeY, self.flakeZ)
...
class EverythingFish(fishTank):
    ...
    def chase(self):
        ...
        if x - self.xPosition > 0: self.xyChange[0] = +1
        elif x - self.xPosition < 0: self.xyChange[0] = -1
        else: self.xyChange[0] = 0

        if y - self.yPosition > 0: self.xyChange[1] = +1
        elif y - self.yPosition < 0: self.xyChange[1] = -1
    ...
    def chaseFood(self):
        self.chase()
        distance = 5
        fooddist = ((self.chaseX-self.xPosition)**2 + (self.chaseY-self.yPosition)**2 +
            (self.chaseZ-self.zPosition)**2)**.5
        if fooddist <= 5:
            self.flake.remove_node()
            self.foodSeq.pause()
            self.move()


    def feed(self, flake, x, y, z):
        # self.flakeList += [[flake, x, y, z]]
        self.flake, self.chaseX, self.chaseY, self.chaseZ = flake,x,y,z
