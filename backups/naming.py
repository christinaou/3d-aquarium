...
loadPrcFileData("", "win-size 2560 1600")
...
class fishTank(ShowBase):
    ...
    def __init__(self):
        ...
        # GUI things
        tangObject = OnscreenImage(image = 'pics/blueTang.png', 
            pos = (0, 0, 0), scale = 0.15, parent = base.a2dBottomLeft)
        sailfinObject = OnscreenImage(image = 'pics/sailfinTang2.png', 
            pos = (-.5, 0, -.5), scale = 0.13)
        yellowObject = OnscreenImage(image = 'pics/yellowTang2.png', 
            pos = (0, 0, -.5), scale = 0.13)
        nemoObject = OnscreenImage(image = 'pics/clownFish4.png', 
            pos = (.5, 0, -.5), scale = 0.13)
    ...
    def setText(self, nameEntered):
        nametxt = OnscreenText(text = nameEntered, pos = (self.topX+.15, +.9),
    ...
    def setFish(self, fishType):
        ...
        if fishType == "blue tang":
            ...
            btObj = OnscreenImage(image = 'pics/blueTang.png', 
            pos = (self.topX, 0, +.9), scale = 0.1)

        if fishType == "sailfin tang":
            x, textt = -.5, "Sailfin Tang"
            sftObj = OnscreenImage(image = 'pics/sailfinTang2.png', 
            pos = (self.topX, 0, +.9), scale = 0.08)

        if fishType == "yellow tang":
            x, textt = 0, "Yellow Tang"
            ytObj = OnscreenImage(image = 'pics/yellowTang2.png', 
            pos = (self.topX, 0, +.9), scale = 0.08)

        if fishType == "clown fish":
            x, textt = .5, "Clown Fish"
            cfObj = OnscreenImage(image = 'pics/clownFish4.png', 
            pos = (self.topX, 0, +.9), scale = 0.08)
        ...
        focusInCommand=self.clearText, pos = (x-.06,0,-.8))

        self.bottX = x
        # Menu for what type of fish
        self.menu = DirectOptionMenu(text="Add", scale=0.05,items=
            ["Community","Aggressive"], highlightColor=(0.65,0.65,0.65,1), 
            pos = (x-.1,0,-.85), command=self.selectType)

    def selectType(self, typeFish):
        self.addButt = DirectButton(text = ("Add"), extraArgs = [typeFish],
            scale = 0.05, command = self.addFish, pos = (self.bottX, 0, -.95))
    ...
    def addFish(self, typeFish):
        if typeFish == "Community":
            self.fishList += [Fish(self.currentFish, self.tankDims, self.currName)]
        elif typeFish == "Aggressive": 
            prevList = self.fishList
            self.fishList += [AggressiveFish(self.currentFish, self.tankDims,
                prevList, self.currName)]
        typetxt = OnscreenText(text = typeFish, pos = (self.topX+.15, +.85),
            scale = 0.05, fg = (1,1,1,1))
        self.currName = ""
    ...
    def stop(self):
        ...
        self.playButt = DirectButton(text = ("Run", "", "Run?", "disabled"), 
            scale = 0.08, command = self.moveFish, pos = (+1, 0, -.8), parent = base.a2dBottomRight)
...
class EverythingFish(fishTank):
    ...
    def __init__(self, typeFish, tankDims, name):
...
class Fish(EverythingFish):
    ...
    def __init__(self, fishType, tankDims, name):
        EverythingFish.__init__(self, fishType, tankDims, name)
...
class AggressiveFish(EverythingFish):
    def __init__(self, fishType, tankDims, fishList, name):
        EverythingFish.__init__(self, fishType, tankDims, name)
