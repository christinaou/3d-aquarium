...
class fishTank(ShowBase):
    ...
    def aqSplash(self):
        ...
        self.createTank()
        self.topFishList = []
        self.makeFishButt = DirectButton(text = ("Add Fish"), parent = base.a2dBottomCenter,
            pos = (0, 0, +.2), command = self.addfishGUI, scale = 0.07)
        self.switchCamButt = DirectButton(text = ("Switch View"), scale = 0.06,
            command = self.switchCam, pos = (-.25, 0, +.3), parent = base.a2dBottomRight)
    ...
    def switchCam(self):
        if base.camera.getPos() == (0,0,200):
            base.camera.setPosHpr(0,-150,38,0,-10,0)
        elif base.camera.getPos() == (0,-150,38):
        ...
        for i in range(len(self.flakeList)):
            ...
            if flake != None:
                ...
                if minDist == None or fooddist < minDist:
                    ...
                    minDist = fooddist
        if self.closestFlake == None or self.closestFlake.flake == None:
            self.chasingFood = False
            for flake in self.flakeList:
                if flake != None:
