...
class EverythingFish(object):
    ...
    def UDLRFish(self, seq):
        # lets us add in a simple turn to move the fish
        # using inheritance

        numm = random.randint(1,3) # turn 2/3 times of the time
        print(numm)
        if numm == 1:
            seq.append(Func(self.leftRightFish))
        # otherwise move up or down
        elif numm == 2:
            seq.append(Func(self.upDownFish))
