...
class fishTank(ShowBase):
    ...
    def evenMoreButtons(self):
        self.ownFishButt = DirectButton(text = ("Add own fish"), scale = 0.06,
            command = self.swimAsFish, pos = (.6, 0,+0), 
            parent = base.a2dBottomCenter)
        ...
        self.moreDirGUI += ([self.fishViewButt] + [self.feedButt] + 
            [self.addButt2] + [self.playButt2] + [self.ownFishButt])
    ...
    def addFish(self, typeFish):
        ...
        elif typeFish == "Own Fish":
            self.fishList += [OwnFish(self.currentFish, self.tankDims,
                prevList, self.currName, self.render2)]
        print("hi")
        ...
        viewButt = DirectButton(text = (self.currName),
            ...
            [len(self.viewButtList)], pos = (self.topX+.15, 0, -.4), 
            parent = base.a2dTopLeft)
        viewButt.hide()
