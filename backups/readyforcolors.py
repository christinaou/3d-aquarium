...
class DirectGUIObjects(object):
    ...
    def DirectGUIText(self):
        ...
        self.pickFish = OnscreenText(text = 
            ...
            fg = (1,1,1,1), scale = 0.05, pos = (+.45,+.2), 
    ...
    def addFish(self, typeFish):
        ...
        if typeFish == "Community":
            ...
            self.fishList += [UserFish(self.currentFish, self.tankDims,
                self.currName, self.render2)]
            self.fishView()
            self.parentCam(len(self.fishList)-1)
        ...
        viewButt = DirectButton(text = (self.currName),
            scale = 0.05, command = self.parentCam, extraArgs = 
            [len(self.fishList)-1], pos = (self.topX+.15, 0, -.4), 
            parent = base.a2dTopLeft)
        viewButt.hide()
